...
func (es *EnvVarsService) CreateByRepoId(ctx context.Context, repoId uint, opt *EnvVarOption) (*EnvVar, *http.Response, error) {
	u, err := urlWithOptions(fmt.Sprintf("/repo/%d/env_vars", repoId), opt)
	if err != nil {
		return nil, nil, err
	}
...
//
// Travis CI API docs: https://developer.travis-ci.com/resource/env_vars#create
func (es *EnvVarsService) CreateByRepoSlug(ctx context.Context, repoSlug string, opt *EnvVarOption) (*EnvVar, *http.Response, error) {
	u, err := urlWithOptions(fmt.Sprintf("/repo/%s/env_vars", url.QueryEscape(repoSlug)), opt)
	if err != nil {
		return nil, nil, err
	}
